...
export function ProjectsTable({
  ...
  const tagColorMap: Record<string, string> = {
    Estudo: "bg-cyan-900/40 text-cyan-400 border-cyan-500/30",
  ...
  return (
    <div className="w-full overflow-x-auto">
      <table className="w-full border-collapse">
        {/* Header */}
        <thead>
          <tr className="border-b-2 border-[#7545d6]/30 bg-gray-900/50">
            <th className="px-6 py-4 text-left text-sm font-bold text-[#7545d6] uppercase tracking-wider">
              Projeto
            </th>
            <th className="px-6 py-4 text-left text-sm font-bold text-[#7545d6] uppercase tracking-wider">
              Categoria
            ...
            <th className="px-6 py-4 text-right text-sm font-bold text-[#7545d6] uppercase tracking-wider">
              Status
            </th>
          </tr>
        </thead>
        ...
        <tbody>
          {projects.map((project, index) => {
            ...
            return (
              <tr
                key={project.id}
                className={`border-b border-[#552ea1]/20 transition-all duration-300 cursor-pointer group ${
                  isHovered
                    ? "bg-slate-800/50 border-[#7545d6]/50"
                    : "bg-slate-900/30 hover:bg-gray-900/30"
              ...
              >
                ...
                <td className="px-6 py-5">
                  <div className="flex items-center gap-3">
                    <div className="text-[#7545d6] group-hover:text-[#552ea1] transition-colors">
                      {project.icon}
                    </div>
                    <span className="font-semibold text-white group-hover:text-[#552ea1] transition-colors">
                      {project.name}
                    </span>
                  </div>
                </td>
                ...
                <td className="px-6 py-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-2 bg-slate-800 border border-cyan-500/20 rounded-sm overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-[#7545d6] to-[#552ea1] transition-all duration-500"
                        style={{
                ...
                <td className="px-6 py-5">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <ChevronRight
                      size={18}
                      className={`text-[#7545d6] transition-all duration-300 ${
