 ...
 */
export function CalendarWithTasks({ month, year, allTasks, onDateClick }: CalendarWithTasksProps) {
  ...
  const getTaskColor = (tag: string): string => {
    ...
    return colors[tag as keyof typeof colors] || 'bg-slate-700/60 border-slate-500/50';
  };
  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-7 gap-1">
        {/* Day Headers */}
        {dayNames.map((day) => (
          <div key={day} className="text-center text-xs font-mono text-slate-500 py-2 font-bold">
            {day}
          </div>
        ))}
        ...
        {calendarDays.map((day, index) => {
          const isToday = new Date().getDate() === day.date && new Date().getMonth() === currentMonth && new Date().getFullYear() === currentYear;
          ...
          return (
            ...
            >
              {/* Date Number */}
              <div className={`text-xs font-mono font-bold mb-1 ${isCurrentMonth ? 'text-slate-300' : 'text-slate-600'}`}>
                {day.date}
              </div>
              ...
              <div className="space-y-1">
                {day.tasks.slice(0, 3).map((task) => (
                  ...
                  >
                    <div className="font-bold text-xs truncate">{task.time}</div>
