...
import { Plus, Filter, Search, Zap, Shield, Heart, BookOpen, Target, Scroll } from 'lucide-react';
...
export default function Projects() {
  ...
  const filters = [
    { id: 'em-progresso', label: 'Em Progresso', count: projects.filter(p => p.status === 'em-progresso').length },
    { id: 'concluido', label: 'Concluído', count: projects.filter(p => p.status === 'concluido').length },
    { id: 'pausado', label: 'Pausado', count: projects.filter(p => p.status === 'pausado').length },
  ];
        ...
        <div className="p-8">
          ...
          <div className="grid grid-cols-4 gap-4 mt-8">
            <div className="bg-slate-900/50 border border-cyan-500/30 rounded-lg p-6 text-center">
              <p className="text-xs font-mono text-slate-500 uppercase mb-2">Total</p>
              <p className="text-3xl font-bold text-cyan-400">{projects.length}</p>
            </div>
            <div className="bg-slate-900/50 border border-cyan-500/30 rounded-lg p-6 text-center">
              <p className="text-xs font-mono text-slate-500 uppercase mb-2">Em Progresso</p>
              <p className="text-3xl font-bold text-blue-400">
                {projects.filter(p => p.status === 'em-progresso').length}
              </p>
            </div>
            <div className="bg-slate-900/50 border border-cyan-500/30 rounded-lg p-6 text-center">
              <p className="text-xs font-mono text-slate-500 uppercase mb-2">Concluídos</p>
              <p className="text-3xl font-bold text-green-400">
                {projects.filter(p => p.status === 'concluido').length}
              </p>
            </div>
            <div className="bg-slate-900/50 border border-cyan-500/30 rounded-lg p-6 text-center">
              <p className="text-xs font-mono text-slate-500 uppercase mb-2">Progresso Médio</p>
