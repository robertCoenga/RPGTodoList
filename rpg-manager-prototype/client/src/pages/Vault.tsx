...
import { Plus, Filter, Search, TrendingUp, TrendingDown, DollarSign } from 'lucide-react';
...
export default function Vault() {
  ...
  const [selectedTab, setSelectedTab] = useState<'mes-atual' | 'todas' | 'planejamento'>('mes-atual');
  const [selectedCategory, setSelectedCategory] = useState<string>('todas');

  const categories = ['Casa', 'Alimentação', 'Transporte', 'Saúde', 'Investimento', 'Lazer', 'Outros'];
  const categoryColors: Record<string, { bg: string; text: string; border: string; chart: string }> = {
    'Casa': { bg: 'bg-blue-900/40', text: 'text-blue-300', border: 'border-blue-500/30', chart: '#3b82f6' },
    'Alimentação': { bg: 'bg-orange-900/40', text: 'text-orange-300', border: 'border-orange-500/30', chart: '#f97316' },
    'Transporte': { bg: 'bg-purple-900/40', text: 'text-purple-300', border: 'border-purple-500/30', chart: '#a855f7' },
    'Saúde': { bg: 'bg-green-900/40', text: 'text-green-300', border: 'border-green-500/30', chart: '#10b981' },
    'Investimento': { bg: 'bg-yellow-900/40', text: 'text-yellow-300', border: 'border-yellow-500/30', chart: '#eab308' },
    'Lazer': { bg: 'bg-pink-900/40', text: 'text-pink-300', border: 'border-pink-500/30', chart: '#ec4899' },
    'Outros': { bg: 'bg-slate-700/40', text: 'text-slate-300', border: 'border-slate-500/30', chart: '#64748b' },
  };

  // Dados de contas
  const allAccounts: Account[] = [
    // Março (Mês Atual)
    { id: '1', description: 'Salário', type: 'ganho', amount: 5000, date: '01/03/2026', status: 'pago', category: 'Investimento', associatedTo: 'Trabalho' },
    { id: '2', description: 'Aluguel', type: 'gasto', amount: 1500, date: '05/03/2026', status: 'pago', category: 'Casa' },
    { id: '3', description: 'Supermercado', type: 'gasto', amount: 800, date: '10/03/2026', status: 'pago', category: 'Alimentação' },
    { id: '4', description: 'Internet', type: 'gasto', amount: 150, date: '12/03/2026', status: 'pendente', category: 'Casa' },
    { id: '5', description: 'Freelance', type: 'ganho', amount: 1200, date: '15/03/2026', status: 'pago', category: 'Investimento', associatedTo: 'Projeto X' },
    { id: '6', description: 'Academia', type: 'gasto', amount: 100, date: '18/03/2026', status: 'planejado', category: 'Saúde', associatedTo: 'Treinar musculação' },
    { id: '7', description: 'Curso Online', type: 'gasto', amount: 300, date: '20/03/2026', status: 'pendente', category: 'Investimento', associatedTo: 'React Avançado' },
    { id: '8', description: 'Bônus', type: 'ganho', amount: 800, date: '25/03/2026', status: 'planejado', category: 'Investimento' },
    { id: '9', description: 'Uber', type: 'gasto', amount: 50, date: '18/03/2026', status: 'pago', category: 'Transporte' },
    { id: '10', description: 'Cinema', type: 'gasto', amount: 60, date: '22/03/2026', status: 'pago', category: 'Lazer' },
    { id: '11', description: 'Farmácia', type: 'gasto', amount: 120, date: '25/03/2026', status: 'pago', category: 'Saúde' },

    // Abril
    { id: '12', description: 'Salário', type: 'ganho', amount: 5000, date: '01/04/2026', status: 'planejado', category: 'Investimento' },
    { id: '13', description: 'Aluguel', type: 'gasto', amount: 1500, date: '05/04/2026', status: 'planejado', category: 'Casa' },

    // Fevereiro
    { id: '14', description: 'Salário', type: 'ganho', amount: 5000, date: '01/02/2026', status: 'pago', category: 'Investimento' },
    { id: '15', description: 'Aluguel', type: 'gasto', amount: 1500, date: '05/02/2026', status: 'pago', category: 'Casa' },
    { id: '16', description: 'Restaurante', type: 'gasto', amount: 750, date: '10/02/2026', status: 'pago', category: 'Alimentação' },
  ];
  ...
  const currentMonthAccounts = allAccounts.filter(a => a.date.includes('/03/'));
  const filteredAccounts = selectedTab === 'mes-atual' 
    ? currentMonthAccounts 
    : selectedTab === 'todas'
    ? allAccounts
    : allAccounts.filter(a => a.status === 'planejado');

  const displayedAccounts = filteredAccounts.filter(a =>
          ...
          <div className="grid grid-cols-4 gap-4 mb-8">
            <div className="bg-gradient-to-br from-green-900/30 to-green-900/10 border border-green-500/30 rounded-lg p-6">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-mono text-green-300 uppercase">Ganho (Mês)</p>
                <TrendingUp size={20} className="text-green-400" />
              </div>
              <p className="text-3xl font-bold text-green-400">R$ {currentMonthGanho.toFixed(2)}</p>
            </div>

            <div className="bg-gradient-to-br from-red-900/30 to-red-900/10 border border-red-500/30 rounded-lg p-6">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-mono text-red-300 uppercase">Gasto (Mês)</p>
                <TrendingDown size={20} className="text-red-400" />
              </div>
              <p className="text-3xl font-bold text-red-400">R$ {currentMonthGasto.toFixed(2)}</p>
            </div>

            <div className="bg-gradient-to-br from-yellow-900/30 to-yellow-900/10 border border-yellow-500/30 rounded-lg p-6">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-mono text-yellow-300 uppercase">Sobra</p>
                <DollarSign size={20} className="text-yellow-400" />
              </div>
              <p className="text-3xl font-bold text-yellow-400">R$ {sobra.toFixed(2)}</p>
            </div>

            <div className="bg-gradient-to-br from-cyan-900/30 to-cyan-900/10 border border-cyan-500/30 rounded-lg p-6">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-mono text-cyan-300 uppercase">Fundo Segurança</p>
                <DollarSign size={20} className="text-cyan-400" />
              </div>
              <p className="text-3xl font-bold text-cyan-400">R$ {fundoSeguranca.toFixed(2)}</p>
            </div>
          </div>
