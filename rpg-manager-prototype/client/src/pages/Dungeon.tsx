...
import { Plus, Trash2, Edit2, AlertCircle, ShoppingCart, Check } from 'lucide-react';
...
export default function Dungeon() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<'todas' | 'essencial' | 'importante' | 'luxo'>('todas');
  const [items, setItems] = useState<StockItem[]>([
    // Essenciais
    { id: '1', name: 'Arroz', inStock: true, quantity: 2, totalQuantity: 5, category: 'essencial', addedToShoppingList: false },
    { id: '2', name: 'Feijão', inStock: true, quantity: 1, totalQuantity: 5, category: 'essencial', addedToShoppingList: false },
    { id: '3', name: 'Leite', inStock: false, quantity: 0, totalQuantity: 3, category: 'essencial', addedToShoppingList: true },
    { id: '4', name: 'Pão', inStock: true, quantity: 1, totalQuantity: 3, category: 'essencial', addedToShoppingList: false },
    { id: '5', name: 'Ovos', inStock: true, quantity: 2, totalQuantity: 12, category: 'essencial', addedToShoppingList: false },
    { id: '6', name: 'Sal', inStock: true, quantity: 1, totalQuantity: 2, category: 'essencial', addedToShoppingList: false },
    { id: '7', name: 'Óleo', inStock: true, quantity: 1, totalQuantity: 2, category: 'essencial', addedToShoppingList: false },
    { id: '8', name: 'Açúcar', inStock: true, quantity: 1, totalQuantity: 3, category: 'essencial', addedToShoppingList: false },

    // Importantes
    { id: '9', name: 'Café', inStock: true, quantity: 1, totalQuantity: 2, category: 'importante', addedToShoppingList: false },
    { id: '10', name: 'Chocolate', inStock: false, quantity: 0, totalQuantity: 2, category: 'importante', addedToShoppingList: false },
    { id: '11', name: 'Biscoito', inStock: true, quantity: 1, totalQuantity: 3, category: 'importante', addedToShoppingList: false },
    { id: '12', name: 'Suco', inStock: true, quantity: 2, totalQuantity: 4, category: 'importante', addedToShoppingList: false },

    // Luxo
    { id: '13', name: 'Sorvete', inStock: false, quantity: 0, totalQuantity: 2, category: 'luxo', addedToShoppingList: false },
    { id: '14', name: 'Refrigerante', inStock: true, quantity: 1, totalQuantity: 3, category: 'luxo', addedToShoppingList: false },
    { id: '15', name: 'Doces', inStock: true, quantity: 2, totalQuantity: 3, category: 'luxo', addedToShoppingList: false },
  ]);

  const categoryColors = {
    'essencial': { bg: 'bg-red-900/30', text: 'text-red-300', border: 'border-red-500/30', icon: '⚔️' },
    'importante': { bg: 'bg-yellow-900/30', text: 'text-yellow-300', border: 'border-yellow-500/30', icon: '🛡️' },
    'luxo': { bg: 'bg-purple-900/30', text: 'text-purple-300', border: 'border-purple-500/30', icon: '✨' },
  };
  ...
  const itemsNeedingRestock = useMemo(() => {
    return items.filter(item => {
      ...
      if (item.category === 'essencial' && percentage < 30 && !item.addedToShoppingList) {
        return true;
      }
      if ((item.category === 'importante' || item.category === 'luxo') && !item.inStock && !item.addedToShoppingList) {
        return true;
      }
      return false;
    });
  ...
  const stats = useMemo(() => {
    ...
    const essentialItems = items.filter(i => i.category === 'essencial').length;
    const essentialInStock = items.filter(i => i.category === 'essencial' && i.inStock).length;
        ...
        <div className="p-8">
          ...
          <div className="grid grid-cols-5 gap-4 mb-8">
            <div className="bg-gradient-to-br from-cyan-900/30 to-cyan-900/10 border border-cyan-500/30 rounded-lg p-4">
              <p className="text-xs font-mono text-cyan-300 uppercase mb-1">Total de Itens</p>
            ...
            <div className="bg-gradient-to-br from-green-900/30 to-green-900/10 border border-green-500/30 rounded-lg p-4">
              <p className="text-xs font-mono text-green-300 uppercase mb-1">Em Estoque</p>
            ...
            <div className="bg-gradient-to-br from-red-900/30 to-red-900/10 border border-red-500/30 rounded-lg p-4">
              <p className="text-xs font-mono text-red-300 uppercase mb-1">Precisa Restock</p>
              <p className="text-2xl font-bold text-red-400">{stats.needsRestock}</p>
            </div>

            <div className="bg-gradient-to-br from-yellow-900/30 to-yellow-900/10 border border-yellow-500/30 rounded-lg p-4">
              <p className="text-xs font-mono text-yellow-300 uppercase mb-1">Essenciais</p>
              <p className="text-2xl font-bold text-yellow-400">{stats.essentialInStock}/{stats.essentialItems}</p>
            </div>

            <div className="bg-gradient-to-br from-purple-900/30 to-purple-900/10 border border-purple-500/30 rounded-lg p-4">
              <p className="text-xs font-mono text-purple-300 uppercase mb-1">Lista de Compras</p>
            ...
            <div className="bg-gradient-to-r from-orange-900/30 to-red-900/30 border border-orange-500/30 rounded-lg p-6 mb-8">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-orange-300 mb-2">⚠️ Itens Precisando de Restock</h3>
                  <p className="text-sm text-slate-300 mb-4">
                    {itemsNeedingRestock.length} item(ns) precisam ser adicionados à lista de compras
                  </p>
            ...
            {filteredItems.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredItems.map((item) => {
                  const percentage = getPercentage(item.quantity, item.totalQuantity);
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mb-3">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className={`text-sm font-bold ${
                            percentage >= 50 ? 'text-green-400' : percentage >= 30 ? 'text-yellow-400' : 'text-red-400'
                          }`}>
                            {percentage}%
                          </span>
                        </div>
                      ...
                      <div className="flex gap-2">
                        {!item.addedToShoppingList && (isOutOfStock || isLowStock) && (
                          <button
                            onClick={() => addToShoppingList(item.id)}
